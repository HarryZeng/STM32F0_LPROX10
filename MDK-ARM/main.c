...
void TIM3_Config(void)
{
    ...
    timer_init_structure.TIM_CounterMode = TIM_CounterMode_Up;      //向上计数模式  
    timer_init_structure.TIM_Period = 6000;                          //每1000 uS触发一次中断,??ADC  
...
void NVIC_Configuration(void)
{
  ...
  NVIC_InitStructure.NVIC_IRQChannel = PVD_IRQn;
	...
	NVIC_InitStructure.NVIC_IRQChannelPriority = 10;                //优先级为10
...
void PVD_init(void)
{
	...
	PWR_PVDLevelConfig(PWR_PVDLevel_7); /*设置PVD电压检测*/
	PWR_PVDCmd(ENABLE);
}
